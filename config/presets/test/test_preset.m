function preset = test_preset()
    ...
    classifier = struct(...
        ...
        'cnn', struct(...    % CNNの設定
            'enable', true, ...
            'architecture', struct(...
                'inputSize', [64 64 1], ...  % 入力サイズ [height width channels]
                'numClasses', 5, ...         % 出力クラス数 (5種類の動作)
                'convLayers', struct(...     % 畳み込み層の設定
            ...
            'lstm', struct(...
                'layers', struct(...
                    'numLayers', 4, ...       % LSTM層の数
                    'hiddenUnits', [256 256 256 64], ... % 各層のユニット数
                    'dropoutRate', 0.5 ...    % ドロップアウト率
                ) ...
            ), ...
            'adaBoost', struct(...
                'numLearningCycles', 100, ... % 学習サイクル数
                'learnerType', 'tree', ...    % 基本学習器のタイプ
                'maxNumSplits', 20 ...        % 決定木の最大分割数
            ) ...
        ), ...
