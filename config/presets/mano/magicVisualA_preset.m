function preset = magicVisualA_preset()
    %% === プリセット情報 ===
    preset_info = struct(...
        'name', 'magicVisualA', ...
        'description', 'mano Experiment preset', ...
        'version', '1.0', ...
        'author', '', ...
        'date', '2024-12-29' ...
    );
...
%     end
    ...
    classifier = struct(...
        ...
        'cnn', struct(...    % CNNの設定
            'enable', false, ...
            'architecture', struct(...
                'inputSize', [28 28 1], ...  % 入力サイズ [height width channels]
                'numClasses', 2, ...         % 出力クラス数
                'layers', struct(...         % 畳み込み層の設定
            ...
            'training', struct(...
                ...
                'initialLearnRate', 0.001, ... % 初期学習率
                'maxEpochs', 30, ...          % 最大エポック数
                'miniBatchSize', 128, ...     % ミニバッチサイズ
                'validation', struct(...
                    'ratio', 0.2, ...         % 検証データの割合
                    'frequency', 30, ...       % 検証頻度
                    'patience', 5 ...          % 早期停止のパラメータ
                ) ...
            ), ...
            'inference', struct(...
                'batchSize', 32, ...          % 推論時のバッチサイズ
                'threshold', 0.5, ...         % 分類閾値
                'useGPU', false ...          % GPU使用の有無
            ) ...
        ), ...
