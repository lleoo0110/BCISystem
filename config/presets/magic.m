    ...
    classifier = struct(...
        ...
        'normalize', struct(...
         ...
         'augmentation', struct(... % データ拡張設定
            ...
            'augmentationRatio', 7, ... % モーターイメージリーデータは不足しがちなため4倍に増強
            'combinationLimit', 3, ... % 一度に適用する手法の上限
            'methods', struct(... % 拡張手法設定
                'noise', struct(... % ノイズ付加
                    'enable', true, ...
                    'types', {{'gaussian', 'pink', 'brown'}}, ... % ブラウンノイズ追加（1/f^2特性）
                ...
                'timeshift', struct(... % 時間シフト
                    ...
                    'perChannel', false, ... % 位相関係を維持
                    'probability', 0.5 ...
                ), ...
                'mirror', struct(... % 振幅反転
                    'enable', true, ... % 有効化（符号の反転はERD/ERSの反転を表現）
                    'probability', 0.2 ... % 低確率で適用
                ), ...
                'channelSwap', struct(... % チャンネル入れ替え
                    'enable', true, ... % メンタルタスクでは左右差が重要なため有効化
                    'pairs', {{...
                        {'F3', 'F4'}, ...   % 前頭部 - 認知制御/実行機能
                ...
                'frequencyNoise', struct(... % 周波数選択的雑音（新規追加）
                    'enable', true, ...
                    'freqBands', {{[8, 13], [13, 30]}}, ... % アルファ・ベータ帯域（メンタルタスク関連）
                    'level', 0.2, ... % 中程度の強度
                    'probability', 0.4 ...
                ), ...
