function preset = magic()
    ...
    % データ収集に関する基本設定
    acquisition = struct(...
        'mode', 'offline', ...           % モード: 'online'/'offline'/'eeglab'
        'save', struct(...             % データ保存設定
    ...
    classifier = struct(...
        ...
        'lstm', struct(...            % LSTM設定
            ...
            'optimization', struct(... % 最適化設定
                'searchAlgorithm', 'lhs', ... % （未実装）探索アルゴリズム: 'lhs'/'random'/'grid'
                'maxTrials', 15, ...
                'searchSpace', struct(...
        ...
        'hybrid', struct(...
            ...
            'architecture', struct(...
                ...
                'cnn', struct(...      % CNN部分の設定
                    'batchNorm', false, ...
                    'convLayers', struct(...
                        'conv1', struct('size', [5 3], 'filters', 32, 'stride', 1, 'padding', 'same'), ...
                        'conv2', struct('size', [3 3], 'filters', 64, 'stride', 1, 'padding', 'same') ...
                    ), ...
                    'poolLayers', struct(...
            ...
            'training', struct(...     % 訓練パラメータ
                ...
                'maxEpochs', 100, ...
                'miniBatchSize', 32, ...     % 小さめのバッチサイズ
                'frequency', 5, ...          % より頻繁な検証
                'patience', 5, ...           % 早期停止の猶予を拡大
                'shuffle', 'every-epoch', ...
                'validation', struct(...
                    'enable', true, ...      % 検証有効
                    'kfold', 5 ...
