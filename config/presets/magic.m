function preset = magic()
    ...
    signal = struct(...
        ...
        'window', struct(...          % 解析窓の設定
            'epochDuration', 2.0, ... % エポック時間長 (0.5-10.0 秒) 
            'timeRange', {{[1.0, 6.0]}}, ... % 時間範囲 [開始時間, 終了時間] (秒)
           ...
           'architecture', struct(...
                ...
                'cnn', struct(...      % CNN部分の設定
                    ...
                    'dropoutLayers', struct(...
                        'dropout1', 0.5 ...  % 論文推奨のドロップアウト率
                    ), ...
                    'fullyConnected', 32 ...  % 論文推奨値
                ), ...
                'lstm', struct(...
                    'batchNorm', true, ...
            ...
            'optimization', struct(...  % 最適化処理のパラメータ
                ...
                'maxTrials', 15, ...          % 最大試行回数
                'evaluationMetric', 'accuracy', ... % 評価指標
                'searchSpace', struct(...     % 探索空間の定義
                    'learningRate', [0.0001, 0.0005], ...  % 論文推奨の低い学習率範囲
                    'miniBatchSize', [32, 64], ...         % バッチサイズ範囲
                    'numConvLayers', [1, 1], ...           % 論文推奨の単一層
                    'cnnFilters', [8, 32], ...             % CNNフィルタ数範囲
                    'filterSize', [3, 5], ...              % フィルタサイズ範囲
                    'lstmUnits', [128, 256], ...           % LSTMユニット数範囲
                    'numLstmLayers', [3, 4], ...           % LSTM層数範囲
                    'dropoutRate', [0.4, 0.6], ...         % ドロップアウト率範囲
