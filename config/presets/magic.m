function preset = magic()
    ...
    signal = struct(...
        ...
        'epoch', struct(...           % エポック化設定
            ...
            'visual', struct(...      % 視覚タスク設定
                ...
                'taskTypes', {{'observation'}}, ... % タスク種類: 'observation'/'imagery'
    ...
    classifier = struct(...
       ...
       'hybrid', struct(...           % ハイブリッドモデル（論文に合わせて最適化）
            'enable', false, ...        % ハイブリッドモデル有効
            'gpu', true, ...
            'optimize', true, ...
            'architecture', struct(...
                ...
                'merge', struct(...    % 特徴統合部分
                    ...
                    'fullyConnected', struct(...
                        'layers', [...
                            struct('units', 64, 'name', 'fc_merge1') ...  % 論文の最適値
                        ], ...
                        'activation', 'relu' ...
                    ), ...
                    'dropout', struct(...
                        'rate', 0.5, ...  % 適切なドロップアウト率
                        'name', 'dropout_merge' ...
                    ), ...
                    'output', struct(...
                        'activation', 'softmax', ...  % ソフトマックス分類（AdaBoostが使えない場合の代替）
                        'name', 'output' ...
                    ) ...
                ) ...
            ), ...
            'training', struct(...     % 訓練パラメータ
                'optimizer', struct(...
                    'type', 'adam', ...
                    'learningRate', 0.0001, ...
                    'beta1', 0.9, ...
            ...
            'optimization', struct(...  % 最適化範囲の設定
                'searchSpace', struct(...
                    'learningRate', [0.0001, 0.0005], ...  % 低い学習率範囲
                    'miniBatchSize', [32, 64], ...
                    'numConvLayers', [1, 1], ...           % 単一層に固定
                    'cnnFilters', [8, 32], ...
                    'filterSize', [3, 5], ...
                    'lstmUnits', [128, 256], ...
                    'numLstmLayers', [3, 4], ... 
